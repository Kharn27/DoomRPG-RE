...
[`MAP1_NATIVE_UNLOCK_STATE.md`](MAP1_NATIVE_UNLOCK_STATE.md) owns the next adjacent world family, `13 / EV_UNLOCK`.

```text
branch = agent/esp32-map1-native-unlock-state
base   = e4fb32f41b7074bbb433e64f4c824edb2167cf50
firmware candidate = e423093c8e17dda1345bebecf721dedf4bbb2002
status = IMPLEMENTED; REAL-CYD HARDWARE VALIDATION PENDING
```

The candidate leaves the proven 120-byte OPEN/LOCKED owner unchanged (`lineStateFNV=e5e74861`) and adds a separate 60-byte one-bit-per-line texture-9/10 overlay. Valid UNLOCK always returns handled=true, clears the native lock bit, conditionally changes effective texture 9->10, and returns sound/entity/view synchronization only as deferred metadata.
...
The real CYD must establish the UNLOCK corpus count, mutation split, texture-state FNV, result FNV, first mutated sample and exact incremental heap cost.
...
Persistent native heap through PR #57:
...
OPEN/CLOSE hardware proof:

```text
refs=71 open=39 close=32
mutated=29 locked=18 alreadyTarget=24 removable=12
rollback=29/29
lineState payload=120 B / actual heap=136 B
first success: cmd3 event1 off2 line459 OPEN 0->1
```

## Remaining MAP_INTRO families

Current candidate owns UNLOCK. If it passes, remaining unowned families are:
...
No later family is pre-authorized. Follow the standard workflow: recover exact merged `main`, inspect legacy semantics, add one bounded permanent owner, hardware-probe on normal `esp32-cyd`, document only after PASS, then merge manually.
