...
#define EXPECTED_BITSET_BYTES 60U
...
typedef struct Esp32Map1LineDoorProbeState_s {
    ...
    int attempted;
    int done;
} Esp32Map1LineDoorProbeState;

typedef struct LineDoorAudit_s {
    uint32_t lineDoorFNV;
    uint32_t refs;
    uint32_t openRefs;
    uint32_t closeRefs;
    uint32_t mutatedRefs;
    uint32_t lockedRefs;
    uint32_t alreadyTargetRefs;
    uint32_t removableRefs;
    uint32_t stateExecutorRefused;
    uint32_t rollbackProofs;
    uint32_t idempotentProof;
    uint32_t lockedGuardProof;
    ...
    uint32_t badOpenIndexRefused;
    uint32_t badLockedIndexRefused;
    EspMapEventDescriptor sampleDescriptor;
    EspMapEventDescriptor unsupportedDescriptor;
    EspMapByteCode sampleCommand;
    EspMapLineDoorResult sampleResult;
    uint32_t sampleMutatedFNV;
...
static Esp32Map1LineDoorProbeState probeState;
...
static uint32_t resultHash(const EspMapLineDoorResult* result) {
    uint32_t hash = 2166136261U;
    ...
    hash = hashByte(hash, result->codeId);
    hash = hashByte(hash, result->openBefore);
    hash = hashByte(hash, result->openAfter);
    hash = hashByte(hash, result->locked);
    hash = hashByte(hash, result->mutated);
    hash = hashByte(hash, result->effectFlags);
    return hashByte(hash, result->removeCommandIfHandled);
}

static int resultIsZero(const EspMapLineDoorResult* result) {
    EspMapLineDoorResult zero;
...
static int boundaryIsSafe(const DoomRPG_t* doomRpg) {
    ...
    return Esp32IntroDispose_isDone() && Esp32Map1BspPass1_isDone() &&
           ...
           Esp32Map1NotebookProbe_isDone() && Esp32Map1KeyGateProbe_isDone() &&
           Esp32Map1PasswordProbe_isDone() &&
           ...
           !EspMapLineState_isReady() && !EspAssetPack_isOpen() &&
...
static int validateLineState(const EspMapLineStateView* state,
                             uint32_t* outFNV,
                             uint32_t* outOpen,
                             uint32_t* outLocked) {
    ...
    if (state == NULL || runtime == NULL || outFNV == NULL ||
        outOpen == NULL || outLocked == NULL ||
        state->lineCount != EXPECTED_LINE_COUNT ||
        state->bitsetBytes != EXPECTED_BITSET_BYTES ||
        state->storageBytes != EXPECTED_STORAGE_BYTES ||
        state->openBits == NULL || state->lockedBits == NULL) return 0;

    for (i = 0U; i < runtime->lineCount; ++i) {
        if (!EspMapRuntime_getLine(i, &line) ||
            !EspMapLineState_getOpen(i, &open) ||
            !EspMapLineState_getLocked(i, &locked) ||
            open != (uint8_t)((line.flags & ESP_MAP_LINE_FLAG_OPEN) != 0U) ||
            locked != (uint8_t)((line.flags & ESP_MAP_LINE_FLAG_LOCKED) != 0U)) {
            return 0;
        }
        openCount += open;
        lockedCount += locked;
    ...
    if (EspMapLineState_getOpen(runtime->lineCount, &open) ||
        EspMapLineState_getLocked(runtime->lineCount, &locked)) return 0;

    for (i = 0U; i < state->bitsetBytes; ++i) hash = hashByte(hash, state->openBits[i]);
    for (i = 0U; i < state->bitsetBytes; ++i) hash = hashByte(hash, state->lockedBits[i]);

    if (state->stateFNV1a != hash || state->openCount != openCount ||
        state->lockedCount != lockedCount) return 0;

    *outFNV = hash;
    *outOpen = openCount;
    *outLocked = lockedCount;
    return 1;
}

static int validateDoorResult(const EspMapEventDescriptor* descriptor,
                              uint32_t commandOffset,
                              const EspMapByteCode* command,
                              uint8_t openBefore,
                              uint8_t locked,
                              EspMapLineDoorStatus status,
                              const EspMapLineDoorResult* result) {
    uint8_t targetOpen;
    ...
    if (result->sourceEventIndex != descriptor->eventIndex ||
        ...
        result->codeId != command->id || result->openBefore != openBefore ||
        result->locked != locked) return 0;

    if (locked != 0U) {
        return status == ESP_MAP_LINE_DOOR_LOCKED &&
               result->openAfter == openBefore && result->soundId == 0U &&
               result->mutated == 0U && result->effectFlags == 0U &&
               result->removeCommandIfHandled == 0U;
    }

    if (openBefore == targetOpen) {
        return status == ESP_MAP_LINE_DOOR_ALREADY_TARGET &&
               result->openAfter == openBefore && result->soundId == 0U &&
               result->mutated == 0U && result->effectFlags == 0U &&
               result->removeCommandIfHandled == 0U;
    }

    return status == ESP_MAP_LINE_DOOR_OK && result->openAfter == targetOpen &&
           result->mutated == 1U &&
           result->effectFlags == ESP_MAP_LINE_EFFECT_ALL &&
           result->soundId == (targetOpen != 0U
                                  ? ESP_MAP_LINE_SOUND_OPEN
                                  : ESP_MAP_LINE_SOUND_CLOSE) &&
           result->removeCommandIfHandled ==
               (uint8_t)((command->arg2 & ESP_MAP_COMMAND_FLAG_REMOVE) != 0U);
}

static int auditDoorCommands(LineDoorAudit* audit, uint32_t initialStateFNV) {
    const EspMapLineStateView* state;
    EspMapEventDescriptor descriptor;
    EspMapEventDescriptor badDescriptor;
    EspMapByteCode command;
    EspMapOpcodeExecResult opcodeResult;
    EspMapLineDoorResult result;
    EspMapLineDoorResult secondResult;
    EspMapLineDoorStatus status;
    uint32_t eventIndex;
    uint32_t commandOffset;
    uint32_t hash = 2166136261U;
    ...
    for (eventIndex = 0U; eventIndex < EXPECTED_EVENT_COUNT; ++eventIndex) {
        ...
        for (commandOffset = 0U; commandOffset < descriptor.commandCount;
            ...
            if (command.id != ESP_MAP_OPCODE_OPENLINE &&
                command.id != ESP_MAP_OPCODE_CLOSELINE) continue;

            if (command.arg1 >= EXPECTED_LINE_COUNT ||
                !EspMapLineState_getOpen(command.arg1, &openBefore) ||
                !EspMapLineState_getLocked(command.arg1, &locked)) return 0;
            ...
            ++audit->refs;
            if (command.id == ESP_MAP_OPCODE_OPENLINE) ++audit->openRefs;
            else ++audit->closeRefs;

            state = EspMapLineState_view();
            if (state == NULL || state->stateFNV1a != initialStateFNV) return 0;
            stateBeforeFNV = state->stateFNV1a;

            status = EspMapLineState_applyDoorCommand(
                &descriptor, commandOffset, &result);
            if (!validateDoorResult(&descriptor, commandOffset, &command,
                                    openBefore, locked, status, &result)) return 0;

            if (status == ESP_MAP_LINE_DOOR_OK) {
                ++audit->mutatedRefs;
                if (result.removeCommandIfHandled != 0U) ++audit->removableRefs;
                state = EspMapLineState_view();
                if (state == NULL || state->stateFNV1a == stateBeforeFNV ||
                    !EspMapLineState_getOpen(command.arg1, &openAfter) ||
                    openAfter != result.openAfter) return 0;
                stateAfterFNV = state->stateFNV1a;
                ...
                if (!audit->haveSample) {
                    ...
                    audit->sampleMutatedFNV = stateAfterFNV;
                    audit->haveSample = 1U;
                }

                if (!EspMapLineState_setOpen(command.arg1, openBefore)) return 0;
                state = EspMapLineState_view();
                if (state == NULL || state->stateFNV1a != initialStateFNV) return 0;
                ++audit->rollbackProofs;
            }
            else if (status == ESP_MAP_LINE_DOOR_LOCKED) {
                ++audit->lockedRefs;
                state = EspMapLineState_view();
                if (state == NULL || state->stateFNV1a != stateBeforeFNV) return 0;
            }
            else if (status == ESP_MAP_LINE_DOOR_ALREADY_TARGET) {
                ++audit->alreadyTargetRefs;
                state = EspMapLineState_view();
                if (state == NULL || state->stateFNV1a != stateBeforeFNV) return 0;
            }
            else {
    ...
    if (audit->refs == 0U || audit->mutatedRefs == 0U ||
        audit->openRefs + audit->closeRefs != audit->refs ||
        audit->mutatedRefs + audit->lockedRefs + audit->alreadyTargetRefs != audit->refs ||
    ...
    audit->lineDoorFNV = hash;

    if (!EspMapLineState_getOpen(audit->sampleResult.lineIndex, &openBefore) ||
        !EspMapLineState_getLocked(audit->sampleResult.lineIndex, &originalLocked) ||
        originalLocked != 0U) return 0;

    /* Real-command idempotence: first transition succeeds, second is a no-op. */
    status = EspMapLineState_applyDoorCommand(
        &audit->sampleDescriptor, audit->sampleOffset, &result);
    if (status != ESP_MAP_LINE_DOOR_OK || result.mutated != 1U) return 0;
    stateAfterFNV = EspMapLineState_view()->stateFNV1a;
    status = EspMapLineState_applyDoorCommand(
        &audit->sampleDescriptor, audit->sampleOffset, &secondResult);
    if (status != ESP_MAP_LINE_DOOR_ALREADY_TARGET || secondResult.mutated != 0U ||
        secondResult.openBefore != secondResult.openAfter ||
        EspMapLineState_view()->stateFNV1a != stateAfterFNV) return 0;
    if (!EspMapLineState_setOpen(audit->sampleResult.lineIndex, openBefore) ||
        EspMapLineState_view()->stateFNV1a != initialStateFNV) return 0;
    audit->idempotentProof = 1U;

    /* The native lock predicate must block the same real command atomically. */
    if (!EspMapLineState_setLocked(audit->sampleResult.lineIndex, 1U)) return 0;
    lockedFNV = EspMapLineState_view()->stateFNV1a;
    status = EspMapLineState_applyDoorCommand(
        &audit->sampleDescriptor, audit->sampleOffset, &result);
    if (status != ESP_MAP_LINE_DOOR_LOCKED || result.locked != 1U ||
        result.mutated != 0U || result.soundId != 0U || result.effectFlags != 0U ||
        EspMapLineState_view()->stateFNV1a != lockedFNV) return 0;
    if (!EspMapLineState_setLocked(audit->sampleResult.lineIndex, originalLocked) ||
        EspMapLineState_view()->stateFNV1a != initialStateFNV) return 0;
    audit->lockedGuardProof = 1U;

    /* Fail closed without mutating the restored native world state. */
    memset(&result, 0xa5, sizeof(result));
    if (EspMapLineState_applyDoorCommand(&audit->unsupportedDescriptor,
                                         audit->unsupportedOffset,
                                         &result) != ESP_MAP_LINE_DOOR_UNSUPPORTED ||
        !resultIsZero(&result) ||
        EspMapLineState_view()->stateFNV1a != initialStateFNV) return 0;
    audit->unsupportedRefused = 1U;

    memset(&result, 0xa5, sizeof(result));
    if (EspMapLineState_applyDoorCommand(&audit->sampleDescriptor,
                                         audit->sampleDescriptor.commandCount,
                                         &result) != ESP_MAP_LINE_DOOR_INVALID ||
    ...
    memset(&result, 0xa5, sizeof(result));
    if (EspMapLineState_applyDoorCommand(&badDescriptor, audit->sampleOffset,
                                         &result) != ESP_MAP_LINE_DOOR_INVALID ||
        !resultIsZero(&result)) return 0;
    audit->badDescriptorRefused = 1U;

    memset(&result, 0xa5, sizeof(result));
    if (EspMapLineState_applyDoorCommand(NULL, 0U, &result) !=
            ESP_MAP_LINE_DOOR_INVALID || !resultIsZero(&result)) return 0;
    audit->nullDescriptorRefused = 1U;

    if (EspMapLineState_applyDoorCommand(&audit->sampleDescriptor,
                                         audit->sampleOffset, NULL) !=
            ESP_MAP_LINE_DOOR_INVALID) return 0;
    audit->nullResultRefused = 1U;

    if (EspMapLineState_setOpen(EXPECTED_LINE_COUNT, 0U)) return 0;
    audit->badOpenIndexRefused = 1U;
    if (EspMapLineState_setLocked(EXPECTED_LINE_COUNT, 0U)) return 0;
    audit->badLockedIndexRefused = 1U;

    return EspMapLineState_view() != NULL &&
           EspMapLineState_view()->stateFNV1a == initialStateFNV;
}
...
void Esp32Map1LineDoorProbe_service(struct DoomRPG_s* doomRpgOpaque) {
    ...
    EspMapLineDoorResult notReadyResult;
    LineDoorAudit audit;
    uint32_t heapBefore;
    ...
    uint32_t lineStateFNV;
    uint32_t initialOpen;
    uint32_t initialLocked;
    uint32_t startedMs;
    uint32_t elapsedMs;
    uint32_t notReadyRefused = 0U;

    if (probeState.done || probeState.attempted || doomRpg == NULL) return;
    if (!Esp32Map1PasswordProbe_isDone()) return;

    if (!probeState.armed) {
        probeState.armed = 1;
        printf("[MAPDOORPROBE] ARMED native PASSWORD owner proven; first native line-world state + OPEN/CLOSE execution starts on next loop service\n");
        return;
    }

    probeState.attempted = 1;
    printf("\n=== Doom RPG ESP32-native MAP_INTRO line door world state ===\n");
    printf("[MAPDOORPROBE] CONTRACT build 2x packed 480-line open/locked state (120B), execute only EV_OPENLINE/EV_CLOSELINE open-bit mutation, return deferred animation/relink/sound effects, rollback every probe mutation; no legacy Render/Entity/world mutation\n");

    if (!boundaryIsSafe(doomRpg)) {
        printf("[MAPDOORPROBE] FAILED unsafe precondition\n");
        return;
    }
    ...
    if (runtime == NULL || mapState == NULL || scriptState == NULL ||
        ...
        printf("[MAPDOORPROBE] FAILED native prerequisites unavailable\n");
        return;
    }

    memset(&notReadyResult, 0xa5, sizeof(notReadyResult));
    if (EspMapLineState_applyDoorCommand(&firstDescriptor, 0U,
                                         &notReadyResult) !=
            ESP_MAP_LINE_DOOR_NOT_READY ||
        !resultIsZero(&notReadyResult)) {
        printf("[MAPDOORPROBE] FAILED not-ready fail-closed\n");
        return;
    }
    notReadyRefused = 1U;
    ...
    if (!EspMapLineState_buildFromRuntime()) {
        printf("[MAPDOORPROBE] FAILED line state build\n");
        return;
    }
    heapAfterBuild = heap8Free();
    lineState = EspMapLineState_view();
    if (!validateLineState(lineState, &lineStateFNV, &initialOpen,
                           &initialLocked)) {
        printf("[MAPDOORPROBE] FAILED line state validation\n");
        return;
    }

    if (!auditDoorCommands(&audit, lineStateFNV)) {
        printf("[MAPDOORPROBE] FAILED door corpus/rollback audit\n");
        return;
    }
    ...
    if (heapBefore < heapAfterBuild || heapAfterBuild != heapAfterAudit) {
        printf("[MAPDOORPROBE] FAILED heap lifecycle before=%u build=%u audit=%u\n",
    ...
    if (heapCost < EXPECTED_STORAGE_BYTES ||
        heapCost > EXPECTED_STORAGE_BYTES + MAX_ALLOCATOR_OVERHEAD) {
        printf("[MAPDOORPROBE] FAILED heap cost=%u payload=%u\n",
               (unsigned int)heapCost,
               (unsigned int)EXPECTED_STORAGE_BYTES);
        return;
    }
    ...
    if (lineState == NULL || lineState->stateFNV1a != lineStateFNV ||
        ...
        doomRpg->game->numEntities != 0 || doomRpg->game->numMonsters != 0) {
        printf("[MAPDOORPROBE] FAILED integrity regression\n");
        return;
    }

    printf("[MAPDOOR] READY refs=%u open=%u close=%u mutated=%u locked=%u alreadyTarget=%u removable=%u resultBytes=%u stateExecRefused=%u lineDoorFNV=%08x elapsed=%ums\n",
           (unsigned int)audit.refs,
           (unsigned int)audit.openRefs,
           (unsigned int)audit.closeRefs,
           (unsigned int)audit.mutatedRefs,
           (unsigned int)audit.lockedRefs,
           (unsigned int)audit.alreadyTargetRefs,
           (unsigned int)audit.removableRefs,
           (unsigned int)sizeof(EspMapLineDoorResult),
           (unsigned int)audit.stateExecutorRefused,
           (unsigned int)audit.lineDoorFNV,
           (unsigned int)elapsedMs);
    printf("[MAPDOOR] SAMPLE cmd=%u event=%u off=%u line=%u opcode=%u open=%u->%u locked=%u sound=%u effects=%02x removeIfHandled=%u\n",
           ...
           (unsigned int)audit.sampleResult.codeId,
           (unsigned int)audit.sampleResult.openBefore,
           (unsigned int)audit.sampleResult.openAfter,
           (unsigned int)audit.sampleResult.locked,
    ...
    printf("[MAPDOOR] WORLD lines=%u bitsetBytes=%u storageBytes=%u initialOpen=%u initialLocked=%u lineStateFNV=%08x mutatedFNV=%08x rollback=%u/%u idempotent=%u lockedGuard=%u\n",
           (unsigned int)lineState->lineCount,
           (unsigned int)lineState->bitsetBytes,
           (unsigned int)lineState->storageBytes,
           (unsigned int)initialOpen,
           (unsigned int)initialLocked,
           (unsigned int)lineStateFNV,
           (unsigned int)audit.sampleMutatedFNV,
           (unsigned int)audit.rollbackProofs,
           (unsigned int)audit.mutatedRefs,
           (unsigned int)audit.idempotentProof,
           (unsigned int)audit.lockedGuardProof);
    printf("[MAPDOOR] FAILCLOSED notReady=%u unsupported=%u badOffset=%u badDescriptor=%u nullDescriptor=%u nullResult=%u badOpenIndex=%u badLockedIndex=%u stateAtomic=yes worldRestored=yes\n",
           ...
           (unsigned int)audit.badOpenIndexRefused,
           (unsigned int)audit.badLockedIndexRefused);
    printf("[MAPDOORPROBE] RAM heap8=%u->%u persistentHeapCost=%u payload=%u allocatorOverhead=%u largest8=%u->%u frameFNV=%08x->%08x arenaFNV=%08x->%08x mapStateFNV=%08x->%08x scriptFNV=%08x->%08x\n",
           (unsigned int)heapBefore,
           (unsigned int)heapAfterAudit,
           (unsigned int)heapCost,
           (unsigned int)EXPECTED_STORAGE_BYTES,
    ...
    printf("[MAPDOORPROBE] LEGACY notebookFNV=%08x->%08x keys=%08x->%08x hudFNV=%08x->%08x passwordCanvasFNV=%08x->%08x continuationFNV=%08x->%08x packIO=no legacyRuntimeClear=yes\n",
    ...
    printf("[MAPDOORPROBE] PARK state=%d page=%d nativeArena=yes nativeTileState=yes nativeEventLookup=yes nativeEventDescriptor=yes nativeScriptState=yes nativeFilter=yes nativeOpcodeExec=yes nativeUiIntent=yes nativeStringReader=yes nativeStatusMessageOwner=yes nativeDialogOwner=yes nativeNotebookOwner=yes nativeKeyGate=yes nativePasswordOwner=yes nativeLineState=yes nativeDoorExec=yes storageBytes=%u resultBytes=%u worldMutationProven=yes worldRestored=yes legacyWorldMutation=no framebufferMutation=no entities=0 monsters=0 noGameplay=yes\n",
