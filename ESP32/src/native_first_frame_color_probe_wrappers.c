...
/*
 * First-frame fidelity diagnostics.
 *
 ...
 * uses stdio/SD and the first fopen() can retain a small libc/VFS allocation,
 * so it runs only AFTER the frame integrity probe has PARKed, never inside the
 * zero-heap-delta renderer contract.
 */
