...
#define EXPECTED_GLOW_DEFERRED 7U
...
void Esp32JunctionSpriteOverlayProbe_service(struct DoomRPG_s* doomRpgBase) {
    ...
    printf("\n=== Doom RPG ESP32-native Junction BSP-visible billboard overlay ===\n");
    printf("[JUNCTIONSPRITE] CONTRACT logical BSP id is sparse ownership key; same stateful compact BSP walk owns wall depth plus visible-leaf admission; only Render_relinkSprite-equivalent candidates enter sort/raster; physical bitshape resolves through bounded mappings.bin ranges; intrinsic legacy mode0 plus ID136/137/144 mode7 additive RGB565 saturation; animationTime0; spawned glow companions still deferred; no resident legacy graphics pools and no world/entity mutation\n");

    if (!EspNativeJunctionSprite_render(render, &stats)) {
        printf("[JUNCTIONSPRITE] FAILED renderer objects=%u bsp=%u/%u unsupported=%u mode0=%u mode7=%u mode7Pixels=%u draws=%u pixels=%u reads=%u glowDeferred=%u\n",
    ...
    if (after == 0U || after == before || stats.objects != 48U ||
        ...
        stats.mode7Pixels == 0U ||
        stats.glowDeferred != EXPECTED_GLOW_DEFERRED ||
        ...
        !legacyClear(render) || EspAssetPack_isOpen()) {
        printf("[JUNCTIONSPRITE] FAILED post frame=%08x->%08x objects=%u bsp=%u/%u hidden=%u unsupported=%u modes=%u/%u mode7Pixels=%u glow=%u draws=%u pixels=%u depth=%u/%u/%u/%u/%u/%u/%u/%u heap8=%u->%u largest8=%u->%u topology=%08x/%08x catalog=%08x/%08x legacyClear=%d pack=%d\n",
               (unsigned int)before,
               (unsigned int)after,
               (unsigned int)stats.objects,
               (unsigned int)stats.bspCandidates,
    ...
    printf("[JUNCTIONSPRITE] READY frame=%08x->%08x objects=%u bspCandidates=%u bspRejected=%u modes=0:%u/7:%u mode7Pixels=%u draws=%u nearCull=%u clipCull=%u spans=%u pixels=%u wallOccludedCols=%u frames=%u uniqueLogical=%u frameBytes=%u maxFrame=%u packReads=%u glowDeferred=%u heapDelta=0 largestDelta=0 legacyRenderStable=yes topology=%08x catalog=%08x packClosed=yes presented=1\n",
           (unsigned int)before,
           (unsigned int)after,
           (unsigned int)stats.objects,
           (unsigned int)stats.bspCandidates,
           ...
           (unsigned int)topologyFNV,
           (unsigned int)catalogFNV);
    printf("[JUNCTIONSPRITE] PARK baseBillboards=yes bspVisibleOnly=yes intrinsicMode7=yes depthBspParity=yes glowPending=yes noLegacyGraphicsPools=yes noWorldMutation=yes\n");
    probeState.done = 1;
}
