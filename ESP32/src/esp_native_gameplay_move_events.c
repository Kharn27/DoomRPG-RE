...
typedef enum MoveCensusKind_e {
    MOVE_CENSUS_NONE = 0,
    MOVE_CENSUS_SHOW = 1,
    MOVE_CENSUS_DIALOG = 2
} MoveCensusKind;

typedef struct MoveCensusPhase_s {
    uint16_t eventIndex;
    uint8_t codeId;
...
static int censusPhase(uint16_t tile,
                       uint32_t runFlags,
                       int initialMode,
                       MoveCensusPhase* outPhase) {
    EspMapEventRef eventRef;
    EspMapEventDescriptor descriptor;
    EspMapEventFilterPlan plan;
    EspMapEventCommandFilterResult filtered;
    ...
    uint8_t eligible = 0U;
    uint8_t showCount = 0U;
    uint8_t firstCode = 0U;
    uint8_t firstOffset = 0U;
    uint32_t offset;

    if (outPhase == NULL) return 0;
    memset(outPhase, 0, sizeof(*outPhase));
    outPhase->eventIndex = UINT16_MAX;

    memset(&eventRef, 0, sizeof(eventRef));
    if (!EspMapEvents_findByTile(tile, &eventRef)) return 1;
    memset(&descriptor, 0, sizeof(descriptor));
    if (!EspMapEvents_describe(&eventRef, &descriptor)) return 0;

    state = descriptor.initialState;
    if (!initialMode &&
        !EspMapScriptState_getEventState(descriptor.eventIndex, &state)) {
        return 0;
    }
    memset(&plan, 0, sizeof(plan));
    if (!EspMapEventFilter_prepare(&descriptor, state, 0U, runFlags, 0U,
                                   &plan)) {
        return 0;
    }
    ...
    for (offset = 0U; offset < descriptor.commandCount; ++offset) {
        ...
        memset(&filtered, 0, sizeof(filtered));
        if (!EspMapEventFilter_evaluate(&descriptor, &plan, offset, removed,
                                        &filtered)) {
            return 0;
        }
        if (filtered.decision != ESP_MAP_EVENT_COMMAND_ELIGIBLE) continue;
        ...
        if (eligible == 0U) {
            firstCode = filtered.codeId;
            firstOffset = filtered.commandOffset;
        }
        if (eligible != UINT8_MAX) ++eligible;

        if (firstCode == ESP_MAP_OPCODE_DIALOG ||
            firstCode == ESP_MAP_OPCODE_DIALOG_NO_BACK) {
            break;
        }
        if (filtered.codeId == ESP_MAP_OPCODE_SHOW && showCount != UINT8_MAX) {
            ++showCount;
        }
    }

    outPhase->eventIndex = descriptor.eventIndex;
    outPhase->codeId = firstCode;
    outPhase->commandOffset = firstOffset;
    outPhase->eligibleCount = eligible;

    if (eligible != 0U && firstCode == ESP_MAP_OPCODE_SHOW &&
        showCount == eligible &&
        showCount <= ESP_NATIVE_GAMEPLAY_MOVE_SHOW_BATCH_MAX) {
        outPhase->kind = MOVE_CENSUS_SHOW;
    }
    else if (eligible != 0U &&
             (firstCode == ESP_MAP_OPCODE_DIALOG ||
              firstCode == ESP_MAP_OPCODE_DIALOG_NO_BACK)) {
        outPhase->kind = MOVE_CENSUS_DIALOG;
    }
    return 1;
}
...
int EspNativeGameplayMoveEvents_logShowDialogCandidates(void) {
    static const struct {
        ...
        { 1,  0, MOVE_EXIT_POS_X | MOVE_BLOCK_INPUT_FLAG,
                  MOVE_ENTER_POS_X | FACING_EAST_FLAG | MOVE_BLOCK_INPUT_FLAG,
                  "E" },
        {-1,  0, MOVE_EXIT_NEG_X | MOVE_BLOCK_INPUT_FLAG,
                  MOVE_ENTER_NEG_X | FACING_WEST_FLAG | MOVE_BLOCK_INPUT_FLAG,
                  "W" },
        { 0, -1, MOVE_EXIT_NEG_Y | MOVE_BLOCK_INPUT_FLAG,
                  MOVE_ENTER_NEG_Y | FACING_NORTH_FLAG | MOVE_BLOCK_INPUT_FLAG,
                  "N" },
        { 0,  1, MOVE_EXIT_POS_Y | MOVE_BLOCK_INPUT_FLAG,
                  MOVE_ENTER_POS_Y | FACING_SOUTH_FLAG | MOVE_BLOCK_INPUT_FLAG,
                  "S" }
    };
    ...
    if (runtime == NULL || runtime->events == NULL ||
        runtime->eventCount == 0U || !EspMapScriptState_isReady()) {
        printf("[MOVEEVENTCENSUS] NOT-READY mutation=no allocation=no\n");
        return 0;
    }
    ...
    for (eventIndex = 0U; eventIndex < runtime->eventCount; ++eventIndex) {
        ...
        if (!EspMapRuntime_getEvent(eventIndex, &raw)) return 0;
        sourceTile = (uint16_t)(raw & ESP_MAP_EVENT_TILE_MASK);
        sx = (uint16_t)(sourceTile & 31U);
        sy = (uint16_t)(sourceTile >> 5U);

        for (d = 0U; d < 4U; ++d) {
            int nx = (int)sx + dirs[d].dx;
            int ny = (int)sy + dirs[d].dy;
            uint16_t destTile;
            MoveCensusPhase sInit, dInit, sNow, dNow;
            int currentMatch;

            if (nx < 0 || nx >= 32 || ny < 0 || ny >= 32) continue;
            destTile = (uint16_t)((uint16_t)ny * 32U + (uint16_t)nx);

            if (!censusPhase(sourceTile, dirs[d].exitFlags, 1, &sInit) ||
                !censusPhase(destTile, dirs[d].enterFlags, 1, &dInit)) {
                return 0;
            }
            if (sInit.kind != MOVE_CENSUS_SHOW ||
                dInit.kind != MOVE_CENSUS_DIALOG) {
                continue;
            }

            if (!censusPhase(sourceTile, dirs[d].exitFlags, 0, &sNow) ||
                !censusPhase(destTile, dirs[d].enterFlags, 0, &dNow)) {
                return 0;
            }
            currentMatch =
                sNow.kind == MOVE_CENSUS_SHOW &&
                dNow.kind == MOVE_CENSUS_DIALOG;

            ++candidates;
            printf("[MOVEEVENTCENSUS] CANDIDATE n=%u dir=%s sourceTile=%u sourceEvent=%u showCount=%u destTile=%u destEvent=%u dialogOpcode=%u dialogCmd=%u initial=yes current=%s mutation=no allocation=no\n",
                   (unsigned int)candidates,
                   dirs[d].name,
                   (unsigned int)sourceTile,
                   (unsigned int)sInit.eventIndex,
                   (unsigned int)sInit.eligibleCount,
                   (unsigned int)destTile,
                   (unsigned int)dInit.eventIndex,
                   (unsigned int)dInit.codeId,
                   (unsigned int)dInit.commandOffset,
                   currentMatch ? "yes" : "no");
        }
    }

    printf("[MOVEEVENTCENSUS] SUMMARY events=%u candidates=%u mode=initial+current mutation=no allocation=no\n",
